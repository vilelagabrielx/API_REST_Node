...
describe('DELETE /deleteapartner/:id', () => {
  // Descrição do teste

  it('Verifica se o parceiro foi apagado', async () => {
    // Descrição do comportamento esperado do teste

    const response = await request(app).delete(`/deleteapartner/${ID}`); // Realiza uma requisição GET para a rota /getpartnerbyid/1
  ...
  it('Verifica a mensagem padrão caso seja passado um id que não existe', async () => {
    const response = await request(app).delete('/deleteapartner/9999999'); // Realiza uma requisição GET para a rota /getpartnerbyid/999
    expect(response.status).toBe(200); // Verifica se o status da resposta é 200
