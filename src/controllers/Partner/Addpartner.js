...
(async () => {
  ...
  try {
    //Inicio da estrutura trycatch mais abrangente
    ...
    try {
      ...
      date = now.format('dd/mm/yyyy hh:mm:ss');
      logger.info(
        `${date} -Erro ao criar tabela de áreas de cobertura - ${Error}`,
      );
    }
    ...
    try {
      ...
      if (Error.message == 'PROCEDURE sp_atualiza_partner already exists') {
        ...
        date = now.format('dd/mm/yyyy hh:mm:ss');
        logger.info(
          `${date} - Erro ao criar procedure de update das tabelas - ${Error}`,
        );
      }
    }
    ...
    date = now.format('dd/mm/yyyy hh:mm:ss');
    logger.info(
      `${date} - Controller/partner.js - Erro ao executar operações iniciais no banco de dados - ${Error}`,
    );
  } finally {
    if (db.connection) {
      ...
      logger.info(
        `${date} - Controller/partner.js - Erro ao se conectar no banco de dados - ${Error}`,
      );
    }
  }
  ...
  async savePartner() {
    try {
      ...
      if (existingPartner.existingPartner == true) {
        throw new Error(
          `Já existe um parceiro com o documento ${this.document}`,
        );
      } else {
        ...
        await db.createCoverageArea(
          partnerId,
          this.coverageArea.type,
          JSON.stringify(this.coverageArea),
        );

        // Insere o parceiro na tabela t_adress

        await db.createAdress(
          partnerId,
          this.address.type,
          JSON.stringify(this.address.coordinates[0]),
          JSON.stringify(this.address.coordinates[1]),
        );
      ...
      logger.info(
        `${date} - Controller/partner.js - Erro ao inserir um novo parter - ${Error}`,
      );
