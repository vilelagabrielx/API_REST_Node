...
class Partner {
  ...
  async UpdatePartner(
    ...
    try {
      ...
      if (existingPartner != false) {
        const rows = await db.UpdatePartnerByID(
          ID,
          tradingName,
          ownerName,
          document,
      ...
      logger.info(
        `${date} - Controller/partner.js - Erro ao procurar parceiro - ${Error}`,
      );
  ...
  async getNearestPartnerByCOORDENATES(X, Y) {
    try {
      ...
      // Registra o erro no arquivo de log
      logger.info(
        `${date} - Controller/partner.js - Erro ao procurar parceiro mais proximo - ${Error}`,
      );
